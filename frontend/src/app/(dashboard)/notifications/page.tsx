...
export default function NotificationsPage() {
  ...
  async function read(n: any) {
    await notifApi.markRead(n.id).catch(() => {}); load();
            ...
            <div key={n.id} onClick={() => read(n)}
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-1">
                  {n.entityType && <Badge variant="outline" className="text-[10px]">{n.entityType}</Badge>}
                  {TYPE_ROUTES[n.type] && <span className="text-[11px] text-green-600 font-medium">Click to open →</span>}
                </div>
              </div>
